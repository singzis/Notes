...
- 当插入顺序是你解决问题时需要考虑的，并且当前需要使用除 String 和 Symbol 以外的键名时，那么 Map就是个最佳解决方案
...
此时用户自定义的 toString 就会破坏到原有的对象 而Map键名接受任何类型，没有影响
...
- 当你需要处理一些属性，那么Object是完全受用的，尤其是需要处理 JSON 数据的时候。由于 Map可以是任意类型，因此没有可以将其转化为 JSON 的原生方法。
...
- 当你需要通正则表达式判断去处理一些业务逻辑时，Map将是你的最佳解决方案
